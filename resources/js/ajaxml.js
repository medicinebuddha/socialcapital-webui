...
function ajaxClick(event) {  
    ...
    var into = $('#'+el.attr('data-ajaxml-into'));
    ...
    loadInto(el.attr('data-ajaxml-into'), url, method, el);
...
function ajaxPlace( element, data ){
    ...
    if (!target.attr('insert') || (target.attr('insert') == 'data-ajaxml-into')) {
        target.html(data);
    }			
 ...
 */	
function ajaxify() {
    $('a[data-ajaxml-into]').removeAttr('onclick');

    $('a[data-ajaxml-into]:not([bind])').each(function(){
        var link = $(this);
        setBinder(this);	
        link.bind('click', ajaxClick); 
    }); 

    $('select[data-ajaxml-into]:not([bind])').each(function(){
        var select = $(this);
        setBinder(this);
        select.bind('change', ajaxClick); 
    }); 

    $('button[data-ajaxml-into]:not([bind])').each(function(){
        var button = $(this);
        setBinder(this);
        button.bind('click', ajaxClick); 
    });
  
    $('input:button[data-ajaxml-into]:not([bind])').each(function(){
        var button = $(this);
        setBinder(this);
        button.bind('click', ajaxClick); 
    });
  
    $('input:checkbox[data-ajaxml-into]:not([bind])').each(function(){
        var checkbox = $(this);
        setBinder(this);
        checkbox.bind('click', ajaxClick); 
    }); 
    $('input:radio[data-ajaxml-into]:not([bind])').each(function(){
        var radio = $(this);
        setBinder(this);
        radio.bind('click', ajaxClick); 
    }); 
    $('input:text[data-ajaxml-into]:not([bind])').each(function(){
        var text = $(this);
        setBinder(this);
    ...
    $('form[data-ajaxml-into]:not([bind])').each(function(){ 
        var form = $(this);
        ...
        if (!hasFileInput || hasHandledFileInput) {
            ...
            form.submit(function(){
                ...
                var target = form.attr('data-ajaxml-into');
                if (target == 'self') 
                    target = form.attr('id');

                evalOnStart(form, form.attr('data-ajaxml-into') );
                ...
                if (form.attr('data-ajaxml-into') == 'self') {
                    if (!form.attr('insert') || form.attr('insert') == '') 
                        form.attr('insert', 'before');
                    ajaxWait(form.attr('id'));
                }
                else {
                    $('#' + form.attr('data-ajaxml-into')).attr('bind', form.attr('bind'));
                    ajaxWait(form.attr('data-ajaxml-into'));
                }
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if (form.attr('data-ajaxml-into') == 'self') {
                            ...
                            ajaxPlace(form.attr('data-ajaxml-into'), data);
                        }

                        ajaxify();
                        evalOnComplete( form, form.attr('data-ajaxml-into') );
                    ...
                    error: function(jqXHR, textStatus, errorThrown){
                        ...
                        evalOnError( form, form.attr('data-ajaxml-into'), jqXHR, textStatus, errorThrown );
...
function bindSubmit(form, bind){
    ...
    form.submit( function(){
        ...
        if (form.attr('data-ajaxml-into') == 'self'){
            ...
            target = form.attr('data-ajaxml-into');
        }
        ...
        fr.load( function(){
            ...
            if (res){
                if (form.attr('data-ajaxml-into')){

                    res = evalOnLoad(form, form.attr('data-ajaxml-into'), res);
...
function addHiddenForm(ctl){
    ...
    if (f){
        ...
        f.attr('data-ajaxml-into', ctl.attr('data-ajaxml-into') || "upload_div"+num);
